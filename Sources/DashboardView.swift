    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 32) {
                if net.isLoading && net.stats == nil {
                    ProgressView().frame(maxWidth: .infinity).padding(.top, 40)
                }
                if let err = net.error {
                    Text(err).font(.caption).foregroundStyle(.secondary).padding()
                }
                ...
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    if let updated = net.lastUpdated {
                ...
                if let s = net.stats {
                    ...
                    let topRepos = s.perRepo.sorted { $0.value > $1.value }.prefix(5)
                    if !topRepos.isEmpty {
                        VStack(spacing: 0) {
                            Divider().opacity(0.15)
                            ForEach(Array(topRepos), id: \.key) { item in
                                HStack {
                                    Text(item.key)
                                        .font(.system(size: 12, design: .monospaced))
                                        .foregroundStyle(.secondary)
                                    Spacer()
                                    Text("\(item.value) \(item.value == 1 ? "commit" : "commits")")
                                        .font(.system(size: 12, design: .monospaced))
                                        .foregroundStyle(.tertiary)
                                        .monospacedDigit()
                                }
